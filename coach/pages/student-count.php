...
// Function to fetch basketball student data without grade level
function getBasketballStudentsData($conn) {
...
// Function to count basketball students (no longer grouped by grade level)
function countBasketballStudents($conn) {
    $sql = "SELECT COUNT(*) as count 
            FROM students s
            JOIN users u ON u.id = s.user_id
            WHERE u.sports_type = 'basketball'";
    $result = $conn->query($sql);
...
$students = getBasketballStudentsData($conn);

// Fetch the total count of basketball students
$totalStudents = countBasketballStudents($conn);
?>

<div class="container-fluid p-0 m-0 profile-page" id="saved" style="display: none;">
  ...
  <form id="attendanceForm" method="post" action="controller/save-student-count.php">
    <table class="table table-bordered">
      ...
      <tbody id="studentDetailsTable">
        <?php
        // Display basketball student full names dynamically with position, status, and attendance dropdown
